    ...
    <div className="p-8 space-y-8 bg-[#F9F6F2] min-h-screen text-[#2C2320]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#6E625C] text-sm mt-1">
            Manage messages and queries submitted through the contact page.
          </p>
        </div>

        <div className="bg-white px-4 py-2 rounded-xl border border-[#EFE8E2] text-sm font-medium">
          Total Messages:{" "}
          <span className="text-[#C89A84] font-bold">
            {inquiries.length}
          </span>
        </div>
      </div>
      ...
      {inquiries.length === 0 ? (
        <div className="bg-white p-12 rounded-3xl border border-[#EFE8E2] text-center space-y-3">
          ...
          <p className="text-[#6E625C] text-sm">
            When customers fill out the contact form, their messages will
            appear right here.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6">
          {inquiries.map((item) => (
            <div
              key={item.id}
              className="bg-white p-6 rounded-2xl border border-[#EFE8E2] shadow-sm space-y-4"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#EFE8E2] pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-[#1F1816] text-lg">
                      {item.name}
                    </h3>

                    <a
                      href={`mailto:${item.email}`}
                      className="text-sm text-[#C89A84] hover:underline"
                    >
                      {item.email}
                    </a>
                  </div>
              ...
              <div className="space-y-1">
                <span className="text-xs font-bold uppercase tracking-wider text-[#6E625C]">
                  Message:
                </span>
