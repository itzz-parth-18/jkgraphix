...
export const metadata: Metadata = {
  ...
  description:
    "Browse our collection of personalized keepsakes, custom memory boxes, and bespoke business printing solutions.",
};
...
export default async function ShopPage({ searchParams }: Props) {
  ...
  // Prisma ke exact enum ko use kar rahe hain.
  const currentType =
    type === "cr"
      ? ProductType.DESIGN_CONSULTATION
      : ProductType.QUICK_CUSTOMIZE;

  // Base Query Setup for Products
  const where: any = {
    status: "PUBLISHED",
  };
  ...
  /*
   * Categories sirf type-specific view mein required hain:
   * - Main hub par category boxes render nahi hote.
   * - Type-specific page par category filter boxes ke liye required hain.
   */
  let dbCategories: {
    id: string;
    name: string;
  ...
  /*
   * Main hub:
   * - No pagination is displayed.
   * - We only need 8 products for the QC/CR showcase.
   *
   * Type-specific page:
   * - Count and products are independent queries, so run them together.
   */
  let totalProducts = 0;
  let products: any[] = [];
  ...
  const qcShowcase = formattedProducts
    .filter(
      (product: any) =>
        product.categoryType === ProductType.QUICK_CUSTOMIZE
    )
    .slice(0, 4);
  ...
  const crShowcase = formattedProducts
    .filter(
      (product: any) =>
        product.categoryType === ProductType.DESIGN_CONSULTATION
    )
    .slice(0, 4);
